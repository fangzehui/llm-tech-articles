...
这是一个值得正面看待的时点。本文给读者三件事：**一份 AB-MCTS 算法 + TreeQuest 源码的精读笔记**（前半，约一半篇幅）、**一张把 TreeQuest / OpenRouter / Vercel AI SDK / LangGraph 四家编排器叠在同一张表上的横评**（后半），以及**一套可以直接 fork 的最小可跑工具集**（[chapter-20-llm-orchestrator-treequest](https://github.com/fangzehui/llm-tech-articles/tree/main/chapters/chapter-20-llm-orchestrator-treequest)）。中间会反复强调一件事——**这 4 家不是"二选一"，而是 4 个正交层（路由 / 前端 / 状态机 / test-time compute），是叠着用的**。
...
把 TreeQuest 的源码摊开来看，主结构非常薄——核心就是 `Node` / `algorithm.step()` / `top_k()` / `render()` 四件事。下面是把官方实现按教学口径浓缩过一遍的最小可读版本（完整版在 [treequest_minimal.py](https://github.com/fangzehui/llm-tech-articles/tree/main/chapters/chapter-20-llm-orchestrator-treequest/treequest_minimal.py)）：
...
实际跑出来的结果（[配套仓库](https://github.com/fangzehui/llm-tech-articles/tree/main/chapters/chapter-20-llm-orchestrator-treequest)里 ``python treequest_minimal.py`` 一行即可复现）：
...
光看表不够直观。配套仓库的 [multi_orchestrator_compare.py](https://github.com/fangzehui/llm-tech-articles/tree/main/chapters/chapter-20-llm-orchestrator-treequest) 实现了 4 种风格在同一个 fake LLM 池上各跑一遍，30 trials 的均值结果：
...
- **本文配套源码**（LLM 编排器工具集：TreeQuest 最小实现 + 4 风格横评 + 成本曲线）：https://github.com/fangzehui/llm-tech-articles/tree/main/chapters/chapter-20-llm-orchestrator-treequest
